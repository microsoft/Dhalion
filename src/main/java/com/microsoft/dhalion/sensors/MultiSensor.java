...
/**
 * The Multi Sensor consumes metrics from the metrics provider and returns them directly in their current form.
 * It can be used when the metrics returned by the metrics provider do not require any further manipulation before
 * being consumed by the detectors. Every time the Multi Sensor is invoked it returns metrics that correspond to the
 * time window between the timestamp of the previous policy invocation and the current timestamp.
 * <p>
 * The Multi Sensor takes as input a collection of metric names, the metrics provider, and a set of components.
 * It collects the values of the given metrics for these components.
 */
public class MultiSensor implements ISensor {
  private static final Logger LOG = Logger.getLogger(MultiSensor.class.getName());
  ...
  public MultiSensor(Config sysconfig, Collection<String> metricNames, MetricsProvider metricsProvider) {
    this(sysconfig, metricNames, metricsProvider, null);
  }

  public MultiSensor(Config policyConf,
